...
Meteor.methods({
  [`${collectionName}.update`](id, unitFields) {
    ...
    const disallowedFieldName = Object.keys(unitFields).find(
      key => !updatableFieldNames.includes(key)
    );
    ...
    if (
      unitFields.unitType &&
      !unitTypes.map(t => t.name).includes(unitFields.unitType)
    ) {
    ...
    if (!metaData.ownerIds.includes(Meteor.userId())) {
      throw new Meteor.Error(
        "You are not one of the owners for this unit, so you are not allowed to update it"
      );
    }
  ...
  [`${collectionName}.updateFloorPlan`](id, floorPlanUrl, dimensions) {
    ...
    if (!unitRole) {
      throw new Meteor.Error(
        "You are not one of the members of this unit, so you are not allowed to update the floor plan"
      );
    }
