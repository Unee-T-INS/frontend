...
import Units, {
  collectionName as unitsCollName,
  getUnitRoles
} from "../../api/units";
import Cases, {
  isClosed,
  severityIndex,
  collectionName as casesCollName
} from "../../api/cases";
import Reports, {
  collectionName as reportsCollName,
  REPORT_DRAFT_STATUS
} from "../../api/reports";
...
import { menuItemDivStyle } from "../general.mui-styles";
...
function NoItem({ item, iconType }) {
  return (
    <div className="mt5 pt3 tc">
      ...
      <div className="mid-gray b lh-copy">You have no {item}s yet</div>
    </div>
  );
}
...
class Unit extends Component {
  ...
  get filteredCases() {
    ...
    const assignedFilter =
      selectedRoleFilter === "Created By Me"
        ? x => x.assignee === this.props.currentUser.bugzillaCreds.login
        : x => true;
    const filteredCases = sortedCases
      .filter(caseItem => assignedFilter(caseItem) && statusFilter(caseItem))
      .sort(sorters[sortBy]);
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (
      ...
      (caseList && caseList.length !== nextProps.caseList.length)
    ) {
      this.setState({
        sortedCases: nextProps.caseList
          .slice()
          .sort(
            (a, b) =>
              severityIndex.indexOf(a.severity) -
              severityIndex.indexOf(b.severity)
          )
      });
    }
  }
  ...
  filteredReports() {
    ...
    const creatorFilter =
      selectedRoleFilter === "Created By Me"
        ? report => report.assignee === currentUser.bugzillaCreds.login
        : report => true;
    const filteredReports = reportList
      .filter(reportItem => creatorFilter(reportItem))
      .sort(sorters[sortBy]);
    return filteredReports.map(({ id, title, status, creation_time: date }) => {
      ...
      return (
        <div
          key={id}
          className="relative bb b--very-light-gray bg-white flex items-center"
        >
          <Link
            to={`/report/${id}/${viewMode}`}
            className="link flex-grow relative w-100"
          >
  ...
  render() {
    ...
    const isOwner =
      metaData.ownerIds && metaData.ownerIds.includes(currentUser._id);
    return (
      <div className="full-height flex flex-column">
        ...
        />
        <Route
          path={`${rootMatch.url}/:viewName`}
          children={({ match }) => {
            const viewIdx = match
              ? viewsOrder.indexOf(match.params.viewName)
              : 0;
            return (
                ...
                <div className="flex-grow flex flex-column overflow-auto">
                  ...
                  >
                            ...
                            <Sorter
                              ...
                              labels={labels.concat([
                                [
                                  ...
                                  {
                                    category: "Updated - Latest",
                                    selected: "Updated ↓"
                                  }
                                ],
                                [
                                  SORT_BY.OLDEST_UPDATE,
                                  {
                                    category: "Updated - Oldest",
                                    selected: "Updated ↑"
                                  }
                                ]
                              ])}
                    ...
                    <div className="flex-grow bg-very-light-gray">
                      ...
                      <Route
                        exact
                        path={`${rootMatch.url}/${viewsOrder[1]}/new`}
                        children={({ match }) => (
                          <CreateReportDialog
                            show={!!match}
                ...
                {editingDetails && (
                  <div className="bg-white scroll-shadow-2 pa3 tc z-999 flex items-center justify-center">
                    <div className="flex-grow">
                      ...
                      >
                        <span className="bondi-blue mh4">Cancel</span>
                      </RaisedButton>
                    </div>
                    <div className="ml3 flex-grow">
                      ...
                      >
                        <span className="white mh4">Save</span>
                      </RaisedButton>
                    </div>
                  </div>
                )}
...
export default connect()(
  createContainer(props => {
    ...
    const unitHandle = Meteor.subscribe(
      `${unitsCollName}.byIdWithUsers`,
      unitId,
      {
        onStop: error => {
          unitError = error;
    ...
    const handles = [unitHandle, Meteor.subscribe("users.myBzLogin")];
    ...
    return {
      ...
      unitUsers: unitHandle.ready()
        ? getUnitRoles(unitItem, Meteor.userId()).map(makeMatchingUser)
        : null,
      caseList: unitHandle.ready()
        ? Cases.find({ selectedUnit: unitItem.name }).fetch()
        : null,
      reportList: unitHandle.ready()
        ? Reports.find({ selectedUnit: unitItem.name }).fetch()
        : null,
