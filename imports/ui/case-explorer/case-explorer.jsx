...
import CaseNotifications, {
  collectionName as notifCollName
} from "../../api/case-notifications";
...
class CaseExplorer extends Component {
  ...
  expandedCasesRenderer = ({ allItems }) => <CaseList allCases={allItems} />;

  handleDialogDismissed = () => this.setState({ showUnitDialog: false });

  makeCaseUpdateTimeDict = memoizeOne(
    allNotifications =>
      allNotifications.reduce((dict, curr) => {
  ...
  makeCaseUnreadDict = memoizeOne(
    unreadNotifs =>
      unreadNotifs.reduce((dict, curr) => {
        const caseIdStr = curr.caseId.toString();
        const unreadItem = (dict[caseIdStr] = dict[caseIdStr] || {
          messages: 0,
          updates: 0
        });
        switch (curr.type) {
  ...
  makeCaseGrouping = memoizeOne(
    ({
      ...
      const assignedFilter =
        selectedRoleFilter !== "Assigned to me"
          ? x => true
      ...
      const unitsDict = cases.reduce((dict, caseItem) => {
        if (assignedFilter(caseItem) && !isClosed(caseItem)) {
          // Filtering only the cases that match the selection
          const { selectedUnit: unitTitle } = caseItem;
          const { bzId, unitType, isActive, displayName } =
            unitsMetaDict[unitTitle] || {};

          // Pulling the existing or creating a new dictionary entry if none
          const unitDesc = (dict[unitTitle] = dict[unitTitle] || {
            cases: [],
            bzId,
            unitType,
            isActive,
          ...
          unitDesc.cases.push(
            Object.assign(
              {
                latestUpdate:
                  caseUpdateTimeDict[caseIdStr] ||
                  new Date(caseItem.creation_time).getTime(),
      ...
      const sortType = sortBy
        ? sorters[sortBy]
        : sorters[SORT_BY.LATEST_UPDATE];
      return Object.keys(unitsDict)
        .reduce((all, unitTitle) => {
          const { bzId, cases, unitType, isActive, displayName } = unitsDict[
            unitTitle
          ];
      ...
      return Object.keys(a).every(key => {
        ...
        if (key === "cases") {
          if (Array.isArray(aAttr) && Array.isArray(bAttr)) {
            return (
              aAttr.length === bAttr.length &&
              aAttr.filter(isClosed).length === bAttr.filter(isClosed).length
            );
          } else {
            return true;
          }
  ...
  filterCases = memoizeOne(
    (cases, searchText) => {
      ...
      const selectedCfg =
        searchCfg[searchText.charAt(0).match(/\d/) ? "num" : "txt"];
      const results = cases.filter(item => {
        return selectedCfg.flds.some(fld => {
          ...
          if (caseResult) {
            if (
              !this.primeCaseId &&
              fld === selectedCfg.redirectOnFieldMatch &&
              caseResult.input === caseResult[0]
            ) {
  ...
  render() {
    ...
    const cases = searchActive
      ? this.filterCases(caseList, searchText)
      : caseList;
          ...
          <Sorter
            ...
            labels={labels.concat([
              [
                ...
                { category: "Updated - Latest", selected: "Updated ↓" }
              ],
              [
                SORT_BY.OLDEST_UPDATE,
                { category: "Updated - Oldest", selected: "Updated ↑" }
              ]
            ])}
          />
        </div>
          ...
          renderAfter={
            <FlatButton primary fullWidth onClick={this.showSelectionDialog}>
              <div className="bondi-blue underline b">
                Add a policy in case management
              </div>
            </FlatButton>
          }
        />
...
const connectedWrapper = connect(
  ...
  createContainer(() => {
    // map meteor state to props
    const casesHandle = Meteor.subscribe(
      `${collectionName}.associatedWithMe`,
      { showOpenOnly: true },
      {
        onStop: error => {
          casesError = error;
    ...
    const isLoading =
      ...
      !notifsHandle.ready() ||
      !unitsHandle.ready() ||
      !userLoginHandle.ready();
    if (isLoading) return { isLoading };

    const caseList = Cases.find().fetch();

    const metaIndex = UnitMetaData.find()
      .fetch()
      .reduce((all, meta) => {
        all[meta.bzId] = meta;
        return all;
    ...
    const unitsMetaGroupings = Units.find()
      .fetch()
      .reduce(
        (all, unitItem) => {
...
connectedWrapper.MobileHeader = connect(({ caseSearchState }) => ({
  searchText: caseSearchState.searchText,
  searchActive: caseSearchState.searchActive
}))(CaseExplorerHeader);
