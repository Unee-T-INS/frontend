...
export default class ChatBotUI extends Component {
  assigneeBot() {
    ...
    return (
      <div>
        You've created a new case! The case has been assigned to{" "}
        <span className="fw5"> {assignedUserRole} </span>and we've notified
        them.
  ...
  inviteBot() {
    ...
    return (
      <Link to={`/case/${caseId}/details/invite`} className="link">
        <div className="tc bondi-blue pv2">Invite People</div>
      </Link>
    );
  }
  ...
  photoBot() {
    return (
      <FileInput onFileSelected={this.props.handleFileSelection}>
        <div className="tc bondi-blue pv2">Add Photo</div>
      </FileInput>
    );
  }
  ...
  render() {
    const chatbotMsg = [
      ...
      {
        id: 2,
        msg: "Would you like to inform someone else about this case?",
        link: this.inviteBot()
      },
      {
        id: 3,
        msg: (
          <span>
            <span className="fw5">Tip: </span> You can also add photos to your
            case so your correspondents have a better understanding of the case
          </span>
        ),
        link: this.photoBot()
      }
        ...
        {chatbotMsg.map(msg => (
          <div className="inline-flex items-end mb3" key={msg.id}>
            ...
            <div className="flex flex-column mr2">
              <div
                className={
                  "mw-60 cf br3 pt2 pl3 pr2 mh2 dib tl bg-white " +
