...
class CaseMaster extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (
      this.matchingPath.path !== nextMatch.path ||
      !_.isEqual(this.matchingPath.params, nextMatch.params)
    ) {
          ...
          <Switch>
            {this.routes.map(({ path, RouteComp, exact = false }) => (
              <Route
                ...
                path={path}
                render={() => (
                  <RouteComp.MobileHeader
                    contentProps={componentsProps[path]}
            ...
            {this.routes.map(({ path, RouteComp, exact }) => (
              <Route
                ...
                render={() => {
                  return (
                    <RouteComp
                      className={isLoading ? "dn" : ""}
                      dispatchLoadingResult={data => {
                        this.setState({
                          componentsData: Object.assign(componentsProps, {
                            [path]: data
                          }),
                          isLoading: false
                        });
                  ...
                  if (match) {
                    ...
                    return (
                      <div className="h-100 w-100">
                        {isSubLoading && <Preloader />}
                        <Case
                          className={isSubLoading ? "dn" : ""}
                          dispatchLoadingResult={() => {
                            this.setState({
                              isSubLoading: false
                            });
                    ...
                    return (
                      <div className="flex flex-column items-center">
                        ...
                        <div className="mt4 roboto f4 mid-gray">
...
export default connect(
  ({ caseSearchState }) => ({ caseSearchState }) // map redux state to props
)(
  createContainer(
    () => ({
      user: Meteor.user() || {}
